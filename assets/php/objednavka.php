...
<head>
  ...
  <style>
    ...
    .email-container { max-width: 520px; margin: 32px auto; background: #FEFBFB; border-radius: 14px; box-shadow: 0 2px 12px rgba(0,0,0,0.07); padding: 40px 32px 32px 32px; }
...
';
    ...
    // --- Zápis do CSV včetně skutečného mail_status ---
    $radek = [
        $orderId,
        date('Y-m-d H:i:s'),
        $template,
    ...
    // --- Odeslání do SheetsDB včetně skutečného mail_status ---
    $sheetsdb_url = 'https://sheetdb.io/api/v1/1e589fzq733tq';
    $data = [
        'cislo_objednavky' => $orderId,
        'datum' => date('Y-m-d H:i:s'),
